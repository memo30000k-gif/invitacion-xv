...
function campoInvitado(invitado = {}) {
    ...
    bloque.innerHTML = `<div class="invitado-encabezado"><strong>Acceso ${numero}</strong><button type="button" class="quitar-invitado" aria-label="Quitar asistente">×</button></div>
        <label>Nombre y apellido<input class="invitado-nombre" minlength="5" maxlength="120" autocomplete="name" placeholder="Ejemplo: María López" value="${(invitado.nombre || "").replace(/&/g,"&amp;").replace(/"/g,"&quot;")}" required></label>
    ...
    const nombre = bloque.querySelector(".invitado-nombre");
...
function nombreCompletoValido(valor) {
    const partes = normalizarNombre(valor).split(" ");
    return partes.length >= 2 && partes.every((parte) => {
        const letras = parte.replace(/[-'’]/g, "");
        return letras.length >= 2 && /^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ'-]+$/.test(parte);
    });
}
...
registro?.addEventListener("submit", async e => {
    e.preventDefault();
    const bloques = [...lista.children];
    const campoInvalido = bloques.map(el => el.querySelector(".invitado-nombre")).find(input => !nombreCompletoValido(input.value));
    if (campoInvalido) {
        campoInvalido.setCustomValidity("Escribe por lo menos un nombre y un apellido, por ejemplo: María López.");
        campoInvalido.reportValidity();
        campoInvalido.focus();
        registroEstado.textContent = "Cada boleto necesita el nombre y por lo menos un apellido.";
        return;
    }
    registroEstado.textContent="Guardando confirmación…";
    const invitados=bloques.map(el=>({id:el.dataset.id||undefined,nombre:normalizarNombre(el.querySelector(".invitado-nombre").value),tipo:el.querySelector(".invitado-tipo").value,bebida:el.querySelector(".invitado-bebida").value}));
